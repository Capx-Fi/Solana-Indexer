...
app.post('/indexer' , async function (request: Request, response: Response, next: NextFunction) {
  try {
    ...
    let user : string = reqBody.user;
    let repo : string = reqBody.repo;
    let branch : string = reqBody.branch;
    let projid : string = reqBody.projid;
    ...
    try {
      ...
      if (error.stderr.toString().includes("branch" + branch + " not found")) {
        ...
        response.status(406).send({status: "Error",error: "Branch not found",statusCode : 406});
        return
