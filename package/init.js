...
function generateProject(answers) {
    ...
    fs.writeFile("../"+answers.INDEXER_NAME+"/indexer.yaml", yaml.dump(data), (err) => {
        if (err) {
            console.log(err);
        }
    });
    ...
    fs.copyFileSync("./generator.ts", "../"+answers.INDEXER_NAME+"/package/generator.ts");
}
...
function createEntitiesYaml(answers) {
    fs.writeFile("../"+answers.INDEXER_NAME+"/entities.yaml" , yaml.dump({
        "entities": [
            {
                "name" : "SampleEntity",
                "params" : [
